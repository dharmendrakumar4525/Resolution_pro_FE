...
const MOMEditor = () => {
  ...
  const countPreviousMeetings = (meetData, selectedId) => {
    ...
    console.log(selectedId, "selectedID", meetData);
    // if (meetData.length<=0) {
    //   console.error("Invalid meetData. Expected an array.");
    //   return;
    // }
        ...
        for (const url of urls) {
          if (url?.title) {
            ...
            if (title === "For #{company_name}") {
              ...
              const momStatements = await resolutionList.filter(
                (data) => data.title == url?.title
              );
              console.log(momStatements[0]?.resolutionUrl, "momStatement");

              combinedContent += momStatements[0]?.momUrl;
              count++;
            }
          }
        }
