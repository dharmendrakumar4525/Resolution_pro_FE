...
export default function ShareholderAttendanceEditor() {
  ...
  const processPlaceholders = (content) => {
    ...
    while ((match = regex.exec(content)) !== null) {
      ...
      console.log(systemVariable, "syste");
  ...
  const handleFileLoad = async (url, specificMeetInfo) => {
    try {
      ...
      <table class="table table-bordered table-hover Master-table" style="width: 100%; border-collapse: collapse;">
        ...
        <tbody>
          ${specificMeetInfo?.shareholder_participants
            ?.map(
              (participant, index) => `
            <tr key="${index}">
              <td>${participant?.shareholder?.name || "Unknown"}</td>
              <td>${
...
`;
  ...
  const saveDocument = async () => {
    ...
    try {
      ...
      if (response.ok) {
        navigate(`/shareholder-documents/${id}?tab=attendance`);
      } else {
