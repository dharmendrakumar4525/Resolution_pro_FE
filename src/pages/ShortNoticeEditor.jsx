...
export default function ShortNoticeEditor() {
  ...
  useEffect(() => {
    const handleMultipleFilesAddOn = async (dynamicResolution) => {
      try {
        ...
        setEditorContent(initializedContent);
      } catch (error) {
  ...
  const saveDocument = async () => {
    ...
    try {
      ...
      if (page === "committee") {
        url = `${apiURL}/committee-meeting/${id}`;
        redirectedUrl = `/committee-documents/${id}?tab=shortNotice`;
      } else if (page === "shareholder") {
        url = `${apiURL}/shareholder-meeting/${id}`;
        redirectedUrl = `/shareholder-documents/${id}?tab=shortNotice`;
      } else {
        url = `${apiURL}/meeting/${id}`;
        redirectedUrl = `/documents/${id}?tab=shortNotice`;
      }
      const response = await fetch(url, {
        method: "PATCH",
        headers: {
